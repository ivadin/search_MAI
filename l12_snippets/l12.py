...
def get_articles_with_snippets(set_of_ids, file_name='doc_id'):
    for doc_id in set_of_ids:
        print("Id: %s." % doc_id, end='')
        title_file_name, url = read_index_title_and_url(doc_id, file_name)
        print(f"Заголовок: {title_file_name}. Url: {url}")
        snippet = get_snippet(title_file_name)
        print(snippet)

    print(f'Articles count: {len(set_of_ids)}')
...
def get_search_res_for_quotes(request):
    ...
    get_articles_with_snippets(set(res_dict))
